  ...
  return (
    <div className="ns-glass-dark rounded-[2rem] border border-white/[0.05] p-8 flex flex-col gap-6">
      ...
      <div>
        ...
        <p className="text-[10px] font-mono text-ns-grey-700 uppercase tracking-widest mt-1">
          {sub}
        </p>
      </div>
    </div>
...
export default function ROICalculator() {
  ...
  const results = useMemo(() => {
    ...
    const savedHoursPerInterview =
      hoursPerInterview * EFFICIENCY_GAIN * teamFactor;
    const savedMonthlyHours =
      interviewers * interviews * savedHoursPerInterview;
    const savedDaysPerMonth = savedMonthlyHours / HOURS_PER_DAY;
    const savedDaysPerYear = savedDaysPerMonth * 12;
    const newInterviewTime =
      hoursPerInterview * (1 - EFFICIENCY_GAIN * teamFactor);
    ...
    return {
      ...
      timeReduction,
      capacityGain: Math.round(
        (savedMonthlyHours / (currentMonthlyHours - savedMonthlyHours + 1)) *
          100,
      ),
    };
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-8 lg:gap-16 mb-16 md:mb-24">
          ...
          <div className="space-y-12">
            <div>
              ...
              <p className="text-sm text-ns-grey-600">
                Adjust the sliders to match your team's hiring metrics.
              </p>
            </div>
            ...
            <div className="text-xs text-ns-grey-700 leading-relaxed space-y-1">
              <p className="font-mono uppercase tracking-widest text-[10px] text-ns-grey-700 mb-2">
                Model assumptions
              </p>
              <p>
                • AI analysis eliminates ~70% of manual code review overhead
              </p>
              <p>
                • Structured session reports cut feedback-writing time by ~60%
              </p>
              <p>
                • Real-time collaboration reduces debrief coordination overhead
              </p>
              <p>
                • Combined effective time reduction: ~
                {Math.round(EFFICIENCY_GAIN * 100)}%
              </p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-sm text-ns-grey-600">
                Estimated impact based on your inputs.
              </p>
            </div>
            ...
            <div className="ns-glass-dark rounded-[2rem] border border-white/[0.05] p-8">
              <p className="text-[10px] font-mono text-ns-grey-600 uppercase tracking-widest mb-4">
                Monthly workload shift
              </p>
              <div className="flex items-center gap-4 mb-3">
                <span className="text-xs text-ns-grey-500 w-24 text-right">
                  Without
                </span>
                ...
                <span className="text-xs text-ns-grey-400 w-16">
                  {results.currentMonthlyHours.toLocaleString()}h
                </span>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 h-2 rounded-full bg-white/[0.06] relative overflow-hidden">
                  <div
                    className="absolute left-0 top-0 h-full rounded-full bg-ns-accent"
                    style={{
                      width: `${Math.max(5, 100 - results.timeReduction)}%`,
                    }}
                  />
                </div>
                <span className="text-xs text-ns-grey-400 w-16">
                  {(
                    results.currentMonthlyHours - results.savedMonthlyHours
                  ).toLocaleString()}
                  h
                </span>
              </div>
